...
class Plotter2D:
    ...
    def __init__(self, x: float, y: float, w: float, h: float, title: str = "Density Profile rho(z)") -> None:
    ...
    def render(
        ...
        # 2. Header Title
        pr.draw_text(self.title.encode("utf-8"), int(self.rect.x + 12), int(self.rect.y + 8), 14, Theme.TEXT_PRIMARY)
        ...
        for i in range(num_grid_x + 1):
            ...
            pr.draw_line(int(gx), int(self.plot_y), int(gx), int(self.plot_y + self.plot_h), Theme.GRID_LINE)
            tick_str = f"{z_val:.1f}".encode("utf-8")
            pr.draw_text(tick_str, int(gx - 10), int(self.plot_y + self.plot_h + 6), 11, Theme.TEXT_MUTED)
        ...
        for j in range(num_grid_y + 1):
            ...
            pr.draw_line(int(self.plot_x), int(gy), int(self.plot_x + self.plot_w), int(gy), Theme.GRID_LINE)
            tick_str = f"{y_val:.1f}".encode("utf-8")
        ...
        if secondary_curves:
            for sec_y, sec_color, sec_name in secondary_curves:
                if len(sec_y) == len(z_arr):
                    ...
                    step = max(1, len(z_arr) // 800)
                    for k in range(0, len(z_arr), step):
                        px, py = self.map_to_screen(z_arr[k], sec_y[k], z_min, z_max, y_min, y_max)
                        px = max(self.plot_x, min(self.plot_x + self.plot_w, px))
                        py = max(self.plot_y, min(self.plot_y + self.plot_h, py))
