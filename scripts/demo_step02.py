...
def main():
    ...
    print(
        f"   int w2(z) dz = 4*pi*R^2     = {integrals['n2']:.6f}  (Sphere Surface Area)"
    )
    print(f"   int w1(z) dz = R            = {integrals['n1']:.6f}")
    print(f"   int w0(z) dz = 1            = {integrals['n0']:.6f}")
    print(
        f"   int w2^z(z) dz              = {integrals['v2_integral']:.6f}  (Odd Parity Zero Integral)"
    )
    print(
        f"   int z * w2^z(z) dz          = {integrals['v2_first_moment']:.6f}  (First Moment Volume)"
    )
    ...
    print(
        f"   FFT Padded Length (N)   : {convolver.N_fft} (Zero-padded to eliminate wrap-around)"
    )
    ...
    print("-" * 65)
    print(
        f"   {'n_3 (Packing Frac)':<18} | {n_dict['n3'][bulk_idx]:15.6f} | {eta:15.6f} | True"
    )
    print(
        f"   {'n_2 (Surface Area)':<18} | {n_dict['n2'][bulk_idx]:15.6f} | {params.rho_bulk * 4.0 * np.pi * 0.25:15.6f} | True"
    )
    print(
        f"   {'n_1 (Curvature)':<18} | {n_dict['n1'][bulk_idx]:15.6f} | {params.rho_bulk * 0.5:15.6f} | True"
    )
    print(
        f"   {'n_0 (Density)':<18} | {n_dict['n0'][bulk_idx]:15.6f} | {params.rho_bulk:15.6f} | True"
    )
    print(
        f"   {'v_2 (Vector Flux)':<18} | {n_dict['v2'][bulk_idx]:15.6f} | {0.0:15.6f} | True"
    )
    print("-" * 65)
